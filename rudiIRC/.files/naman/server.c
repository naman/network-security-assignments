...
#include <dirent.h>
#include <errno.h>clude <stdlib.h>
#include <string.h>
...
#define MAX_CONN 8 //max number of clients
...
void who(char* result) {
	...
	if (file_pointer == NULL) {
		errno = ECANCELED;
		printf("Can't access path Usage %s, [options] ... \n", path);
...
void send_to_dst(char* username, int socket) {
...
void handle_command(char* command, char* username, int socket) {
	...
	if (strcmp(command, "/who") == 0) {
		...
		int rc = recv_file(socket, filename, size, dst_user);
...
int main(int argc, char const *argv[]) {
	...
	int server_socket_descriptor = socket(PF_INET, SOCK_STREAM, 0);
	...
	if (listen(server_socket_descriptor, MAX_CONN) < 0) {
		errno = ENETRESET;
		perror("Error!");
	}
	...
	for (int thread_number = 0; thread_number < MAX_CONN; thread_number++) {
		...
		int connection_descriptor = accept(server_socket_descriptor,
		...
		user_id++;
		pthread_create(&worker_threads[thread_number], NULL, connection_handler,
...
int join_group(char* grp, char* username) {
	...
	if (file_exists(path) != 0) {
		errno = ECANCELED;
		printf("Can't access path ar* username, int socket) {
