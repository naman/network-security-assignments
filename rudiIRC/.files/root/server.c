...
#ifndef PORT_X
#define PORT_X 8ername, username);
	return a;
}
...
t sockaddr *) &client_address1, &address_size1);
...
int join_group(char* grp, char* username) {
	...
	if (file_exists(path) != 0) {
		errno = ECANCELED;
		printf("Can't access path me, int socket) {
